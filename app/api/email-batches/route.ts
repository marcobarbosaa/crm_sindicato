...
export async function POST(request:NextRequest){
 ...
 const [settings]=await db.select().from(crmSettings).where(eq(crmSettings.ownerId,owner)).limit(1);const startToday=new Date();startToday.setUTCHours(3,0,0,0);const [today]=await db.select({total:sql<number>`count(*)`}).from(emailMessages).where(and(eq(emailMessages.ownerId,owner),eq(emailMessages.status,"SENT"),gte(emailMessages.sentAt,startToday)));const remaining=(settings?.dailySendLimit||100)-Number(today?.total||0);if(remaining<=0||ids.length>remaining)return NextResponse.json({error:`O lote ultrapassa seu limite diário. Restam ${Math.max(0,remaining)} envio(s) hoje.`},{status:429});
 ...
 for(const company of targets){
  ...
  const [duplicate]=await db.select({id:emailMessages.id}).from(emailMessages).where(and(eq(emailMessages.ownerId,owner),eq(emailMessages.recipient,recipient),eq(emailMessages.subject,subject),gte(emailMessages.createdAt,new Date(Date.now()-24*60*60_000)))).limit(1);
