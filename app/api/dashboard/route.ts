...
export async function GET(request: NextRequest) {
  ...
  const [companyCounts, todayEmails, weekEmails, dueFollowUps, activities] = await Promise.all([
    ...
    db.select({ total: sql<number>`count(*)` }).from(emailMessages).where(and(eq(emailMessages.ownerId, owner), gte(emailMessages.sentAt, startToday))),
    db.select({ total: sql<number>`count(*)` }).from(emailMessages).where(and(eq(emailMessages.ownerId, owner), gte(emailMessages.sentAt, sevenDaysAgo))),
    db.select({ total: sql<number>`count(*)` }).from(followUps).where(and(eq(followUps.ownerId, owner), eq(followUps.status, "PENDING"),lte(followUps.dueAt,endToday))),
